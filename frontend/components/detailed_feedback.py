# This file is a Streamlit component for rendering detailed resume feedback.
# It takes the backend’s per‑issue analysis and displays it in a structured, styled way, grouped by severity.
...
def _group_by_severity(issues: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for issue in issues:
        ...
        # grouped is a dictionary where keys are severity levels ("critical", "high", "medium", "low"). Values are lists of issues belonging to that severity.
        # setdefault(level, []) :- Checks if the key level exists in the dictionary. If it does, returns its current value (the list of issues).
        # If it doesn’t, creates a new entry with that key and sets its value to [] (an empty list).
        # This ensures there’s always a list available to append to, even if the severity wasn’t predefined.
...
def _render_issue(issue: Dict[str, Any]) -> None:
    ...
    with st.expander("Details", expanded=False):
        ...
        if example:
            ...
            # st.code(...) :- Streamlit function that renders a block of code in the UI.
            # Displays it in a styled box with monospaced font and optional syntax highlighting.
            # Useful for showing examples, snippets, or formatted text.
            st.code(example, language="text")
            # Suppose example = "Skills: Python, Machine Learning".
            # The UI will show:
            # Skills: Python, Machine Learning
            # inside a gray code-style box, making it stand out from normal text.
